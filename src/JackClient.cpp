...
JackClient::JackClient(std::string_view name, Config &config)
    : m_clientName(std::string(name)), m_client(nullptr), m_bufSize(0), m_sampleRate(0),
      m_config(config), wavFile(nullptr), m_peakProcessor("teste", 0.01f), 
      m_fftProcessor(1024), m_dtProcessor(1024)
{
...
void JackClient::Activate()
{
    ...
    {
        throw std::runtime_error(
            fmt::format("Activating client '{}' failed with {}", m_clientName, res));
    }
}
...
JackClient::PortSet JackClient::GetOutPorts(const std::vector<std::string> &filter,
                                            bool connectedOnly)
{
    PortSet result;
    ...
    else
    {
        ...
        {
            auto ports = jack_get_ports(m_client, pattern.c_str(), JACK_DEFAULT_AUDIO_TYPE,
                                        JackPortIsOutput);
            try_insert(ports);
        }
    }
...
int JackClient::OnJackProcess(jack_nframes_t nFrames, void *arg)
{
    ...
    {
        float *buffer =
            static_cast<float *>(jack_port_get_buffer(self->m_pairs[0].first.second, nFrames));
    ...
    else
    {
        ...
        try
        {
            ...
            {
                auto buffer =
                    static_cast<float *>(jack_port_get_buffer(pair.first.second, nFrames));
...
void JackClient::ProcessWavData(float* buffer, size_t frames) {
    size_t available_frames = wavBuffer.size();
    if (available_frames < frames) {
        // If we don't have enough frames to fill the buffer, zero out the remainder.
        std::copy(wavBuffer.begin(), wavBuffer.end(), buffer);
        std::fill(buffer + available_frames, buffer + frames, 0.0f);
    } else {
        // If we have enough frames, copy the data to the buffer.
        std::copy(wavBuffer.begin(), wavBuffer.begin() + frames, buffer);
        wavBuffer.erase(wavBuffer.begin(), wavBuffer.begin() + frames);
    }

    // Ensure the peak processor handles the buffer data properly.
    m_peakProcessor.Process(buffer, static_cast<uint32_t>(frames));
    m_fftProcessor.Process(buffer, static_cast<uint32_t>(frames));
    m_dtProcessor.Process(buffer, static_cast<uint32_t>(frames));
...
size_t JackClient::getWavBufferSize() const 
{
    return wavBuffer.size();  // Return the size of the loaded WAV buffer
}
